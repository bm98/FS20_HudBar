...
namespace FS20_HudBar.Bar.Items
{
  class DI_Com2 : DispItem
  {
    ...
    public DI_Com2( ValueItemCat vCat, Label lblProto, Label valueProto, Label value2Proto, Label signProto, bool showUnits )
    {
      LabelID = LItem;
    ...
    private void _value1_MouseWheel( object sender, MouseEventArgs e )
    {
      ...
      if ( e.Delta > 0 ) {
        SC.SimConnectClient.Instance.ComModule.COM2set( whole ? FSimClientIF.CmdMode.Inc : FSimClientIF.CmdMode.Inc_Fract );
      }
      else if ( e.Delta < 0 ) {
        SC.SimConnectClient.Instance.ComModule.COM2set( whole ? FSimClientIF.CmdMode.Dec : FSimClientIF.CmdMode.Dec_Fract );
      }
    }
    ...
    private void _label_Click( object sender, EventArgs e )
    {
      ...
      SC.SimConnectClient.Instance.ComModule.COM2set( FSimClientIF.CmdMode.Toggle );
    }
    ...
    {
      if ( this.Visible ) {
        _value1.Text = $"{SC.SimConnectClient.Instance.ComModule.COM2_stdby_hz / 1_000_000f:000.000}";
        _value2.Text = $"{SC.SimConnectClient.Instance.ComModule.COM2_active_hz / 1_000_000f:000.000}";
      }
    }
