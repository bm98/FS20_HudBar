...
namespace FS20_HudBar.Bar.Items
{
  class DI_RTrim : DispItem
  {
    ...
    private void _label_MouseWheel( object sender, MouseEventArgs e )
    {
      ...
      float current = SV.Get<float>( SItem.fGS_Trm_RudderTrim_prct100 );
      if (e.Delta > 0) {
        // Wheel Up - yaw Right (same as mouse in Sim)
        SV.Set( SItem.fGS_Trm_RudderTrim_prct100, current + c_incPerWheel );
      }
      else if (e.Delta < 0) {
    ...
    private void _label_ButtonClicked( object sender, ClickedEventArgs e )
    {
      if (SC.SimConnectClient.Instance.IsConnected) {
        SV.Set( SItem.fGS_Trm_RudderTrim_prct100, 0f ); // Set 0
      }
    }
    ...
    {
      if (this.Visible) {
        _value1.Value = SV.Get<float>( SItem.fGS_Trm_RudderTrim_prct100 ) * 100f;
      }
    }
