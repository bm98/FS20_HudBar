...
{
  internal class DI_Nav2_Course : DispItem
  {
    /// <summary>
    /// The Label ID 
    /// </summary>
    public static readonly LItem LItem = LItem.NAV2_OBS;
    /// <summary>
    /// The GUI Name
    /// </summary>
    public static readonly string Short = "CRS 2";
    /// <summary>
    /// The Configuration Description
    /// </summary>
    public static readonly string Desc = "NAV-2 Course/Deviation";
    ...
    private void _value1_MouseClick( object sender, MouseEventArgs e )
    {
      ...
      SV.Set( SItem.bS_Nav_2_OBS_Synch, true );
    }
    ...
    private void _value1_MouseWheel( object sender, MouseEventArgs e )
    {
      ...
      if (e.Delta > 0) {
        float obiSet = (float)CoordLib.Geo.Wrap360( SV.Get<float>( SItem.fGS_Nav_2_OBS_deg ) + (whole ? 5f : 1f) );
        SV.Set( SItem.fGS_Nav_2_OBS_deg, obiSet );
    ...
    private void OnDataArrival( string dataRefName )
    {
      if (this.Visible) {
        ...
        // Has NAV2
        if (SV.Get<bool>( SItem.bG_Nav_2_hasSignal )) {
          // desired heading to the VOR
          _value1.Value = SV.Get<float>( SItem.fGS_Nav_2_OBS_deg );
          float err = SV.Get<float>( SItem.fG_Nav_2_RadialError_deg );
