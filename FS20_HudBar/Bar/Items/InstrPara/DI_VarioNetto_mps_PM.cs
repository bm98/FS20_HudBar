...
{
  class DI_VarioNetto_mps_PM : DispItem
  {
    /// <summary>
    /// The Label ID 
    /// </summary>
    public static readonly LItem LItem = LItem.NETTO_MPS;
    /// <summary>
    /// The GUI Name
    /// </summary>
    ...
    /// </summary>
    public static readonly string Desc = "Netto-Vario + Avg [m/s]";

    private readonly B_Base _label;
    private readonly V_Base _value1;
    private readonly V_Base _value2;

    public DI_VarioNetto_mps_PM( ValueItemCat vCat, Label lblProto, Label valueProto, Label value2Proto, Label signProto )
    {
      LabelID = LItem;
      var item = VItem.NETTO_MPS;
      _label = new B_Text( item, lblProto ) { Text = Short }; this.AddItem( _label );
      _label.ButtonClicked += DI_Vario_ButtonClicked;
      _value1 = new V_VSpeed_mPsPM( valueProto );
      this.AddItem( _value1 ); vCat.AddLbl( item, _value1 );

      item = VItem.NETTO_MPS_AVG;
    ...
    private void OnDataArrival( string dataRefName )
    {
      if (this.Visible) {
        ...
        _value2.Value = (float)(Math.Round( SV.Get<float>( SItem.fG_Acft_VARIO_Avg_netto_mps ) * 10.0 ) / 10.0);
