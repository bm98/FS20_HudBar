  ...
  class DI_N2Graph : DispItem
  {
    ...
    private void OnDataArrival( string dataRefName )
    {
      if (this.Visible) {
        ...
        if (_scale1.Visible) {
          _scale1.Value = SV.Get<float>( SItem.fG_Eng_T1_N2_prct ); // 0..100
          _scale1.ItemBackColor = (SV.Get<float>( SItem.fG_Eng_T1_N2_prct ) > 102) ? cWarnBG : cBG;
        }

        if (_scale2.Visible && !(_scale3.Visible || _scale4.Visible)) {
          _scale2.Value = SV.Get<float>( SItem.fG_Eng_T1_N2_prct ); // 0..100
          _scale2.ValueLScale = SV.Get<float>( SItem.fG_Eng_T2_N2_prct ); // 0..100
          _scale2.ItemBackColor = ((SV.Get<float>( SItem.fG_Eng_T1_N2_prct ) > 102) ||
                                    (SV.Get<float>( SItem.fG_Eng_T2_N2_prct ) > 102)) ? cWarnBG : cBG;
        }

        if (_scale3.Visible) {
          // reorder for 3 Engines
          _scale2.Value = SV.Get<float>( SItem.fG_Eng_T1_N2_prct ); // 0..100
          _scale2.ValueLScale = SV.Get<float>( SItem.fG_Eng_T3_N2_prct ); // 0..100
          _scale2.ItemBackColor = ((SV.Get<float>( SItem.fG_Eng_T1_N2_prct ) > 102) ||
                                    (SV.Get<float>( SItem.fG_Eng_T3_N2_prct ) > 102)) ? cWarnBG : cBG;

          _scale3.Value = SV.Get<float>( SItem.fG_Eng_T2_N2_prct ); // 0..100
          _scale3.ItemBackColor = (SV.Get<float>( SItem.fG_Eng_T2_N2_prct ) > 102) ? cWarnBG : cBG;
        }
        ...
        if (_scale4.Visible) {
          ...
          _scale4.Value = SV.Get<float>( SItem.fG_Eng_T2_N2_prct ); // 0..100
          _scale4.ValueLScale = SV.Get<float>( SItem.fG_Eng_T4_N2_prct ); // 0..100
          _scale4.ItemBackColor = ((SV.Get<float>( SItem.fG_Eng_T2_N2_prct ) > 102) ||
                                    (SV.Get<float>( SItem.fG_Eng_T4_N2_prct ) > 102)) ? cWarnBG : cBG;
        }
      }
    }
