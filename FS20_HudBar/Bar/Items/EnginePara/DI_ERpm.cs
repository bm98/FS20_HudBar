...
namespace FS20_HudBar.Bar.Items
{
  class DI_ERpm : DispItem
  {
    ...
    {
      if (this.Visible) {
        _value1.Value = SV.Get<float>( SItem.fG_Eng_E1_rpm );
        _value2.Value = SV.Get<float>( SItem.fG_Eng_E2_rpm );
        _value3.Value = SV.Get<float>( SItem.fG_Eng_E3_rpm );
        _value4.Value = SV.Get<float>( SItem.fG_Eng_E4_rpm );
      }
    }
