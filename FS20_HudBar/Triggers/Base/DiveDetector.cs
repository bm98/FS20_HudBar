...
{
  /// <summary>
  /// Detects when a value gets below the Level with reset margin
  ///  it will stay true until a value is outside the reset band for AutoReset
  ///  or until Reset by the user
  ///  
  /// </summary>
  internal class DiveDetector : LevelDetectorBase<float>
  {
    // hidden cTor:
    private DiveDetector( ) : base( false ) { }
    ...
    public DiveDetector( float level, float resetBand, bool autoReset )
      : base( autoReset )
    {
      ResetBand = resetBand;
    ...
    public override float Level {
      ...
      set {
        _level = value;
        _slopeDetector = new SlopeDetector<float>( Slope.FromAbove, _level );
